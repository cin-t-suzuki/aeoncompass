...
@section('title', '宿泊体験通知メール設定')
@inject('service', 'App\Http\Controllers\ctl\HtlAlertMailVoiceController')
@section('content')
...
<br>
{!! Form::open(['route' => ['ctl.htl_alert_mail_voice.create'], 'method' => 'get']) !!}
...
@if(count($a_alert_mail_hotel['values']) == 0)
    <ul>
        <small>
            <li>宿泊体験になった際に通知する電子メールアドレスを設定します。
        </small>
    </ul>
@else
    <hr size=1>
    ...
    @foreach($a_alert_mail_hotel['values'] as $alert_mail_hotel)
        <tr>
            ...
            {!! Form::open(['route' => ['ctl.htl_alert_mail_voice.update'], 'method' => 'get']) !!}
                <td align="center">
                    <table>
                        <tr>
                            <td align="center">
            ...
            {{ Form::close() }}
            {!! Form::open(['route' => ['ctl.htl_alert_mail_voice.delete'], 'method' => 'get']) !!}
                <td>
