      ...
      <tr>
        ...
        <td>
          <select size="1" name="priority[pref_id]">
            ...
            {{-- null追記 --}}
            <option value="{{strip_tags($value['pref_id']??null)}}" @if ($value['pref_id']??null == $views->select_pref['pref_id']??null) selected="selected"@endif @if (($value['pref_id'] == 13) && $service->is_empty($views->select_pref['pref_id']??null))selected="selected"@endif>{{strip_tags($value['pref_nm']??null)}}</option>
