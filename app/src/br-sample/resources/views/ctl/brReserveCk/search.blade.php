...
<table border="1" cellpadding="4" cellspacing="0">
  ...
  @foreach ($views->hotel_lists['values'] as $key => $reserve_data)
  <tr>
    {!! Form::open(['route' => ['ctl.brReserveCk.reserveck'], 'method' => 'post', 'target' => 'hotel']) !!}

      @if ($views->date_ymd['after'] == '2009-06-30')
    ...
    {!! Form::open(['route' => ['ctl.brReserveCk.update'], 'method' => 'post']) !!}
      <td nowrap>
       @if ($views->send_customers_ymd <= now() && now() <= $views->dead_line_ymd)
         ...
         @else
            ...
            <INPUT TYPE="hidden" NAME="checksheet_fix[checksheet_ym]" VALUE="{{strip_tags($views->checksheet_ym)}}">
            <input type="hidden" name="Search[keywords]" value="{{strip_tags($views->search['keywords'])}}">
            <input type="hidden" name="Search[year]" value="{{strip_tags($views->search['year'])}}">
            <input type="hidden" name="Search[month]" value="{{strip_tags($views->search['month'])}}">
         @endif
        @else
          @if ($reserve_data->fix_status == 1)
           チェック完了
