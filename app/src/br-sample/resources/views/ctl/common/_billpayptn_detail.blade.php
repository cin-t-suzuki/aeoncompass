  <tr>
    ...
    <th rowspan="2">成約料金 @if ($customer['billpay_ptn_cd']=='P20140400001P')（税抜）@else（税込）@endif <br />消費税</th>
  ...
  {{-- @foreach ($detail['values'] as $detail) --}}
  ...
  <tr>
      ...
      <td class="charge">@if ($customer['billpay_ptn_cd']=='P20140400001P'){{number_format($detail['bill_charge']-$detail['bill_charge_tax'])}} @else {{number_format($detail['bill_charge'])}} @endif <br />{{number_format($detail['bill_charge_tax'])}}</td>
    @if ($customer['document_type'] == 2 || $customer['document_type'] == 3)
