  ...
  {{-- ヘッダ表示 --}}
  <tr>
      <th colspan="@if ($views->customer['document_type'] == 3) 12 @elseif ($views->customer['document_type'] == 1) 8 @else 7 @endif">
        @if ($service->is_empty($views->customer['billpay_ptn_cd']))<span style="font-weight:bold">未精算（繰越）</span>
        @else
          <span style="float:right">No.{{$views->customer['billpay_ptn_cd']}}</span>
          @if (!$service->is_empty($views->customer['bill_ymd']))
            <span style="float:left">
              {{-- {if $v->helper->date->set($v->assign->customer.bill_ymd*1)}{/if}お振込期日 {$v->helper->date->to_format('Y年m月d日')} --}}
              お振込期日 @include ('ctl.common._date',['timestamp' => $views->customer['bill_ymd'] , 'format' => 'ymd'])
            </span>
          @endif
  ...
  <tr>
    ...
    @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
      <th colspan="@if ($views->customer['document_type'] == 1)6 @else 4 @endif">ベストリザーブ社ご請求項目</th>
    @endif
    <th rowspan="2">
    @if (!$service->is_empty($views->customer['billpay_ptn_cd']))
      {{-- <form style="margin:0;" method="post" action="{$v->env.source_path}{$v->env.module}/{$v->env.controller}/book/" target="_blank"> --}}
      {{ Form::open(['route' => 'ctl.brbillpayptn.book', 'method' => 'post', 'target' => '_blank', 'style' => 'margin:0;']) }}
        <input type="submit" value="@if ($views->customer['billpay_type'] == 0) 請求書 @else 支払通知書 @endif" />
        {{-- null追記 --}}
        @if (!$service->is_empty($views->customer['site_cd']??null))
          <input type="hidden" name="site_cd"        value="{{$views->customer['site_cd']}}" />
        @endif
        ...
        <input type="hidden" name="key"            value="{{$views->customer['book_path_encrypt']}}" />
      {{ Form::close() }}
    @endif
    </th>
  </tr>
  ...
  <tr>
    @if ($views->customer['document_type'] == 2 || $views->customer['document_type'] == 3)
      ...
      <th>支払小計</th>
    @endif
    @if ($views->customer['document_type'] == 1)
      <th>宿泊泊数</th>
      <th>取消泊数</th>
    @endif
    @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
  ...
  @foreach ($views->book as $site_cd => $sites)
    ...
    <tr>
      {{-- affiliate_cd_subのみ??null追記 --}}
        <td colspan="@if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 2) 7 @else 10 @endif">{{$sites[0]['site_nm']}} 様 （{{$sites[0]['partner_cd']}}{{$sites[0]['affiliate_cd']}}@if (!$service->is_empty($sites[0]['affiliate_cd_sub']??null))-{{$sites[0]['affiliate_cd_sub']}}@endif）</td>
    </tr>
    ...
    @foreach ($sites as $site)
      <tr>
        ...
        <td style="text-align:center;">
          ...
          {{ Form::open(['route' => 'ctl.brbillpayptn.detail', 'method' => 'post', 'style' => 'margin:0;']) }}
    ...
    {{-- サイト合計行表示 --}}
    @if ($views->customer['document_type'] == 3)
      <tr>
        <td>計</td>
        @if ($views->customer['document_type'] == 2 || $views->customer['document_type'] == 3)
        ...
        @endif
        @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
          <td colspan="@if ($views->customer['document_type'] == 1) 5 @else 3 @endif" class="charge">{{number_format($bill_charge)}}</td>
        @endif
        <td></td>
      </tr>
    @endif
    <tr>
      ...
      @endif
      @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
        <td colspan="@if ($views->customer['document_type'] == 1) 5 @else 3 @endif" class="charge">@if ($billpay_charge >= 0){{number_format($billpay_charge)}} @endif</td>
      @endif
      <td></td>
    </tr>
    {{-- 行間 --}}
    <tr>
        <th colspan="@if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 2) 7 @else 10 @endif"></th>
    </tr>
    {{-- 総合計加算 --}}
    @if ($billpay_charge >= 0)
  ...
  @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
    <tr >
      <td>ご請求合計</td>
       ...
       @endif
       <td colspan="@if ($views->customer['document_type'] == 1) 5 @else 3 @endif" class="charge">{{number_format($billpay_charge)}}</td>
       <td></td>
    </tr>
  @endif
  <tr>
    ...
    @endif
    @if ($views->customer['document_type'] == 1 || $views->customer['document_type'] == 3)
      <td colspan="@if ($views->customer['document_type'] == 1) 5 @else 3 @endif" class="charge">@if ($total_billpay_charge >= 0){{number_format($total_billpay_charge)}}@endif</td>
    @endif
    <td></td>
  </tr>
