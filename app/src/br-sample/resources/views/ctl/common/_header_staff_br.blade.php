  ...
  <div class="hd-base" id="hd-staff-br">
      ...
      <div id="hd-staff-htl">
        ...
        <p>
          ...
          {{-- ハイランクホテルの場合 --}}
          @if ($v->user->hotel_control->stock_type == 1)<span class="msg-text-info">[買]</span>@endif
          
          {{-- プレミアムパッケージ適用施設 --}}
          @if ($v->user->hotel->premium_status)<span class="msg-text-info">[プ]</span>@endif
          
          {{--ヴィジュアルパッケージ適用施設 --}}
          @if ($v->user->hotel->visual_package_status)<span class="msg-text-info">[ヴィ]</span>@endif
          
          {{-- 日本旅行移行施設 --}}
          @if ($v->user->hotel->ydp2_status)<span class="msg-text-info">[日]</span>@endif
        </p>
