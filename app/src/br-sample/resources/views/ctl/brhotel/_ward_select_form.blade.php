...
@if(isset($mast_wards['values']) && count($mast_wards['values']) != 0)
	<select size="1" name="Hotel[ward_id]">
		...
		@foreach ($mast_wards['values'] as $mast_ward)
			<option value="{{strip_tags($mast_ward['ward_id'])}}"
			@if(isset($hotel) && !$service->is_empty($hotel['ward_id']) && $mast_ward['ward_id'] == $hotel['ward_id'])
					selected
			@endif>

			{{strip_tags($mast_ward['ward_nm'])}}</option>
		@endforeach
	</select>
@endif
