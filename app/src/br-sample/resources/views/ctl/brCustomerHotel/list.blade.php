...
<p><table border="1" cellpadding="4" cellspacing="0">
  <tr>
    ...
    <td nowrap><small>{{strip_tags($views->hotel['hotel_cd'])}}</small><br />{{strip_tags($views->hotel['hotel_nm'])}}<br><small>@if (!$service->is_empty($views->hotel['tel']))TEL : {{strip_tags($views->hotel['tel'])}}&nbsp;@endif @if (!$service->is_empty($views->hotel['fax']))FAX : {{strip_tags($views->hotel['fax'])}}@endif</small></td>
  </tr>
</table></p>
    ...
    <tr>
      ...
      <td>
        <select size="1" name="like_type">
          ...
          <option value="front"@if ($views->like_type == "front") selected @endif>前方一致</option>
          <option value="back"@if ($views->like_type == "back") selected @endif>後方一致</option>
        </select>
        <INPUT TYPE="text" NAME="keyword" SIZE="30" MAXLENGTH="30" VALUE="{{strip_tags($views->keyword)}}">
        <INPUT TYPE="submit" VALUE="検索">
        <small>最大
          <INPUT TYPE="text" NAME="limit" SIZE="4" MAXLENGTH="4" VALUE="{{strip_tags($views->limit)}}">
          件表示
        </small>
      </td>
    </tr>
...
{!! Form::close() !!}
@if ($views->customer_list['cnt'] < 1)
@else
  {{strip_tags($views->customer_list['cnt'])}}件見つかりました。
  @if ($views->limit > 0)
    ...
    @for ($customer_list = 0; $customer_list < count($views->customer_list['values']) && $customer_list <= $views->limit ; $customer_list++)
      <tr>
        {!! Form::open(['route' => ['ctl.brCustomerHotel.setting'], 'method' => 'post']) !!}
          <input type="hidden" name="target_cd" value="{{strip_tags($views->target_cd)}}" />
          <input type="hidden" name="limit" value="{{strip_tags($views->limit)}}" />
          <input type="hidden" name="keyword" value="{{strip_tags($views->keyword)}}" />
          <input type="hidden" name="like_type" value="{{strip_tags($views->like_type)}}" />
...
@endif
<br />
@if (count($views->customer_list['values']) > $views->limit)
  <font color="#ff0000">{{strip_tags($views->limit)}}件を越えたので表示を中断しました。</font>
  <br />
@endif
<hr size="1">
<div align="right">
