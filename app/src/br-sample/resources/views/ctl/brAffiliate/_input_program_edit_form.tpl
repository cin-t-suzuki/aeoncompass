  <table border="1" cellpadding="4" cellspacing="0">
    ...
    <tr>
      ...
      <td nowrap>
        {$v->helper->form->strip_tags($v->assign->affiliate_program.reserve_system)}
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td nowrap  bgcolor="#EEFFEE" >表示レイアウトバージョン</td>
      <td nowrap>
          <input type='radio' name="Affiliate_Program[report_layout_version]" id="report_layout_version1" value="1" {if $v->assign->affiliate_program.report_layout_version == 1} checked=""{/if}/><label for="report_layout_version1">旧バージョン</label>
          <input type='radio' name="Affiliate_Program[report_layout_version]" id="report_layout_version2" value="2" {if $v->assign->affiliate_program.report_layout_version == 2} checked=""{/if}/><label for="report_layout_version2">新バージョン</label>
      </td>
    </tr>
  </table>
<input type="hidden" name="Affiliate_Program[affiliater_cd]" value="{$v->helper->form->strip_tags($v->assign->affiliate_program.affiliater_cd)}">
<input type="hidden" name="Affiliate_Program[affiliate_cd]" value="{$v->helper->form->strip_tags($v->assign->affiliate_program.affiliate_cd)}">
<input type="hidden" name="Affiliate_Program[reserve_system]" value="{$v->helper->form->strip_tags($v->assign->affiliate_program.reserve_system)}">