...
<div id="page_top_symbol">
  ...
  {else}
<div>
    <form action="{$v->env.source_path}{$v->env.module}/htlsmedia/list/" method="post" style="display:inline;">
  ...
  {foreach from=$v->assign->medias item=media name=loop_media}
    {assign var="foreach_count_minus" value=$smarty.foreach.loop_media.index-1}
    {assign var="foreach_count_plus"  value=$smarty.foreach.loop_media.index+1}
    ...
    <tr>
      <td style="{$disp_order_width}">
        {if $smarty.foreach.loop_media.first}
          <form action="{$v->env.source_path}{$v->env.module}/htlsmedia/sortmedia/" method="post" style="display:inline;">
            ...
            <input type="hidden" name="edit_order_no" value="{$v->helper->form->strip_tags($v->assign->medias[$foreach_count_plus].media_no)}" />
            <input type="submit" value="↓" />
            <input type="hidden" name="change_flg" value="down">
        ...
        {elseif $smarty.foreach.loop_media.last}
          <form action="{$v->env.source_path}{$v->env.module}/htlsmedia/sortmedia/" method="post" style="display:inline;">
            ...
            <input type="hidden" name="edit_order_no" value="{$v->helper->form->strip_tags($v->assign->medias[$foreach_count_minus].media_no)}" />
            <input type="submit" value="↑" />
            <input type="hidden" name="change_flg" value="up">
            <input type="hidden" name="label_type"        value="{$v->assign->form_params.label_type}"       />
            <input type="hidden" name="label_cd[outside]" value="{$v->assign->form_params.label_cd.outside}" />
            <input type="hidden" name="label_cd[map]"     value="{$v->assign->form_params.label_cd.map}"     />
            <input type="hidden" name="label_cd[inside]"  value="{$v->assign->form_params.label_cd.inside}"  />
      ...
      <td>
        <table border="1" cellspacing="0" cellpadding="6" width="100%">
          <tr>
            ...
            <td style="{$update_width}">{$media.upload_dtm|date_format:'%Y-%m-%d'}</td>
            {/if}
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>{$media.upload_dtm|date_format:'%Y-%m-%d'}</td>
          </tr>
          {/if}
        </table>
      </td>
      ...
      <td>
        <form action="{$v->env.source_path}{$v->env.module}/htlsmedia/editmedia/" method="post" style="display:inline;">
          <input type="hidden" name="target_cd"        value="{$v->assign->form_params.target_cd}" />
          <input type="hidden" name="media_no"         value="{$media.media_no}" />
      ...
      <td>
        <form action="{$v->env.source_path}{$v->env.module}/htlsmedia/destroymedia/" method="post" style="display:inline;" onSubmit="return is_confirm();" >
          <input type="hidden" name="target_cd"        value="{$v->assign->form_params.target_cd}" />
          <input type="hidden" name="media_no"         value="{$media.media_no}" />
