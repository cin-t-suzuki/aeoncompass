...
{include file='../_common/_br_header2.tpl' title="パートナー精算サイト一覧"}
    ...
    { foreach from=$v->assign->sites item=site name=sites}
        ...
        <tr>
            <td>{$site.site_cd}</td>
            <td>{$site.site_nm}
                {if !is_empty($site.person_post)}<br />{$site.person_post}{/if}
                {if !is_empty($site.person_nm)}<br />{$site.person_nm} 様{/if}
            </td>
            <td>通知方法：{if ($site.mail_send==1)}メールで通知する{else}通知しない{/if}<br />
                email:{$site.email_decrypt|replace:',':'<br />'}<br />
            </td>
            <td>
                {if !is_empty($site.partner_cd)}パートナー<br />{$site.partner_nm}（{$site.partner_cd}）{/if}
                {if !is_empty($site.affiliate_cd)}アフィリエイト<br />{$site.affiliate_nm}（{$site.affiliate_cd}）{/if}
            </td>
            <td>
                {if is_empty($site.sales_customer_id) and is_empty($site.stock_customer_id)}
                    {if !is_empty($site.partner_cd)}料率タイプを設定してください。{/if}
                    {if !is_empty($site.affiliate_cd)}指定なし{/if}
                {/if}
                ...
                {if !is_empty($site.sales_customer_id)}
                    {if !is_empty($site.stock_customer_id)}<hr />{/if}
