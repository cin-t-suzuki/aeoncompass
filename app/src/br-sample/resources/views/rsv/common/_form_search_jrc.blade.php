...
<form method="get" action="/jrc/{$v->assign->pre_uri}" target="_blank" class="parseForm" >
    <table border="0" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <th><div>出発日</div></th>
        <td>
          <select size="1" name="year_month"></select>
          <select size="1" name="day" class="text-right"></select>&nbsp;
        </td>
      </tr>
      <tr>
        <th><div>１室人数</div></th>
        <td>
          <select size="1" name="guest" class="text-right">
      {section name=senior start=1 loop=7}
        <option value="{$smarty.section.senior.index}"{
          if $smarty.section.senior.index == $v->assign->senior} selected="selected"{
...
{if is_empty($v->hotel.hotel_cd)}
    <div class="sfm-jrc-submit">
          <div class="btn-b06-138-sb" style="margin:0 auto;">
            <input class="btnimg collectBtn collectForce" type="image" src="{$v->env.root_path}img/btn/b06-jrc1.gif" alt="ＪＲ＋宿泊検索" />
          </div>
    </div>
...
{else}
    <div class="sfm-jrc-submit">
          <div class="btn-b06-138-sb" style="margin:0 auto;">
            <input class="btnimg collectBtn collectForce" type="image" src="{$v->env.root_path}img/btn/b06-jrc3.gif" alt="ＪＲ＋宿泊検索へすすむ" />
          </div>
    </div>
    <div style="text-align:center;">この商品は株式会社日本旅行が企画・実施しております。</div>
    <input type="hidden" name="hotel_cd" value="{$v->hotel.jrc_hotel_cd}" />
{/if}
