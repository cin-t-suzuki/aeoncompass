...
{assign var=url_query_hotel value=$v->env.path_base|cat:'/query/?type=hotel&'|cat:$v->helper->form->to_query_correct('type,place,hotel_cd,hotel_cds,plan_id,room_id,x,y',false)}
...
{foreach from=$v->assign->index.hotels key=hotel_no item=hotel name=hotels}
  ...
  <div class="hi-box advance">
    <div class="hi-hotel border-bd" style="padding:5px;">
      <div class="hi-name
      ...
      {if $plan_count==0 and $vhotel.icons.ads} bg-ranktop {if ($vhotel.hotel_nm|count_characters:true) >= 40} bg-ranktop_h {/if}{assign var=priority_count value=1}{/if}
      "><div style="display: flex; align-items: center;"><a href="{$v->env.path_base}/plan/{$hotel.hotel_cd}/">{$v->helper->form->strip_tags($vhotel.hotel_nm)}</a>
      ...
      {if $priority_count > 0}
      {if ($vhotel.hotel_nm|count_characters:true) >= 40}</div><div>{* brが効かないので無理やり改行 *}{/if}
      <span style="margin-left:auto;" class="bg-ranktop_msg" >☆☆☆【注目】☆☆☆</span>
      {/if}
      </div>
      {if ($vhotel.hotel_nm|count_characters:true) >= 40}<br>{/if}{* display flexを使っているとその中で改行しても高さが変わらないのでここで改行している *}
      </div>
            ...
            <div class="hi-cmd">
              ...
              {if $v->assign->params.view != "marker"}
                {if $vhotel.icons.cliped}
                 <div class="btn-b06-clip2-s"><a class="btnimg jqs-clip jqs-clip-{$hotel.hotel_cd} jqs-on" href="/clip/{$hotel.hotel_cd}/" title="クリップ削除"><img src="/img/btn/b06-clip2.gif" width="98" height="23" alt="クリップ中" title="「クリップ」とは、よく泊まる宿泊施設や、いつか泊まってみたい気になる宿泊施設を保存できる機能です。サイトに会員ログインいただき「クリップ」すると、その宿泊施設はベストリザーブ・宿ぷらざの画面を閉じても、あらためてサイトに会員ログインいただくと「クリップ」されたまま保存されています。「クリップ」された宿泊施設だけの空室を簡単に探すことができるようになります。" /></a></div>
                {else}
                 <div class="btn-b06-098-s"><a class="btnimg jqs-clip jqs-clip-{$hotel.hotel_cd}" href="/clip/{$hotel.hotel_cd}/" title="クリップする"><img src="/img/btn/b06-clip1.gif" width="98" height="23" alt="クリップする" title="「クリップ」とは、よく泊まる宿泊施設や、いつか泊まってみたい気になる宿泊施設を保存できる機能です。サイトに会員ログインいただき「クリップ」すると、その宿泊施設はベストリザーブ・宿ぷらざの画面を閉じても、あらためてサイトに会員ログインいただくと「クリップ」されたまま保存されています。「クリップ」された宿泊施設だけの空室を簡単に探すことができるようになります。" /></a></div>
                {/if}
              {/if}
            </div>
          </td>
    ...
    {foreach from=$hotel.plans key=plan_no item=plan name=plans}
      ...
      ">
        <div class="pi-head{if $vplan.icons.point_rate > 9} pi-head-p{$vplan.icons.point_rate}{/if}{if $vplan.icons.highrank} pi-head-ph{/if}{if !($vplan.icons.highrank) and $priority_count>0} pi-head-pr{/if}">
          ...
          <div class="pi-icons">
            ...
            {* 最低宿泊日数・最大宿泊日数 *}
            {if $vplan.icons.stay_limit > 1 and $plan.icons.stay_cap > 1}
        ...
        {foreach from=$plan.plan_rooms key=room_no item=room name=rooms}
          ...
          {assign var=room_count value=$room_count+1}

          <div class="ri-box gi-box border-bd">
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                  ...
                  <td class="gi-price gi-pricefix" nowrap="nowrap">
                    ...
                    {/if}
                    <br />
                    合計<span class="gi-currency">{$charge.total_sales_charge|number_format}円</span><br />
                  </td>
                      ...
                      <div class="btn-b04-098-s">
                        {if $v->user->partner->connect_type == 'clutch' && !($v->user->member->is_login())}
                          <form method="post" action="{$v->user->partner->request_url}">
                            ...
                            <input type="hidden" name="furl" value="{$v->env.port_https}{$v->env.path_base}/booking/step0/" />
      ...
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
        ...
        {* すべての部屋を表示するリンク *}
        {if $room_count < $vplan.room_count}
          <div class="pi-foot">
            <div class="pi-link"><a href="{$url_query_hotel}&hotel_cd={$hotel.hotel_cd}&plan_id={$plan.plan_id}">このプランのすべての部屋を見る</a></div>
          </div>
        {/if}
