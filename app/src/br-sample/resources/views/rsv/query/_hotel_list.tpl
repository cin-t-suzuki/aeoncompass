...
{foreach from=$v->assign->index.hotels key=hotel_no item=hotel name=hotels}
  ...
  <div class="hi-box advance">
    {if ($v->assign->search_condition.form.hotel.hotel_cd|count_characters != 10) or $v->assign->params.view == 'marker' }{* 施設が固定されている場合は施設情報を表示しないただし地図のバルーンクリック時を除く *}
    <div class="hi-hotel">
      <div class="hi-name
            ...
            <div class="hi-cmd">
              ...
              {if $v->assign->params.view != "marker"}
                {if $vhotel.icons.cliped}
                 <div class="btn-b06-clip2-s"><a class="btnimg jqs-clip jqs-clip-{$hotel.hotel_cd} jqs-on" href="{$v->env.path_img}/clip/{$hotel.hotel_cd}/" title="クリップ削除"><img src="/img/btn/b06-clip2.gif" width="98" height="23" alt="クリップ中" title="「クリップ」とは、よく泊まる宿泊施設や、いつか泊まってみたい気になる宿泊施設を保存できる機能です。サイトに会員ログインいただき「クリップ」すると、その宿泊施設はベストリザーブ・宿ぷらざの画面を閉じても、あらためてサイトに会員ログインいただくと「クリップ」されたまま保存されています。「クリップ」された宿泊施設だけの空室を簡単に探すことができるようになります。" /></a></div>
                {else}
                 <div class="btn-b06-098-s"><a class="btnimg jqs-clip jqs-clip-{$hotel.hotel_cd}" href="{$v->env.path_img}/clip/{$hotel.hotel_cd}/" title="クリップする"><img src="/img/btn/b06-clip1.gif" width="98" height="23" alt="クリップする" title="「クリップ」とは、よく泊まる宿泊施設や、いつか泊まってみたい気になる宿泊施設を保存できる機能です。サイトに会員ログインいただき「クリップ」すると、その宿泊施設はベストリザーブ・宿ぷらざの画面を閉じても、あらためてサイトに会員ログインいただくと「クリップ」されたまま保存されています。「クリップ」された宿泊施設だけの空室を簡単に探すことができるようになります。" /></a></div>
                {/if}
              {/if}
              {* ＪＲコレクション プラン一覧へのリンク *}
            ...
            <td>
              ...
              <div class="pi-head{if $vplan.icons.point_rate > 9} pi-head-p{$vplan.icons.point_rate}{/if}{if $vplan.icons.highrank} pi-head-ph{/if}">
                ...
                <div class="pi-icons">
                  ...
                  {* ポイント利用 *}
                  {if $vplan.icons.point_status}<a href="{$v->env.port_https}{$v->env.path_base}/point/"><img src="{$v->env.path_img}/spc/spc-p0-point.gif" width="85" height="20" alt="ポイント利用可" /></a>{/if}
                  ...
                  {* GoToキャンペーン *}
                  {if $vplan.icons.camp_goto}<img src="{$v->env.root_path}img/spc/spc-p0-camp_goto_plan.gif" width="140" height="20" alt="GoToトラベルキャンペーン" />{/if}
