...
<div class="sfm-keyword">
    ...
    <div class="sfm-keyword-inner">
        ...
        <div class="btn-b01-068-s" style="margin:-8px 16px 0 0; float:right; padding:5px;">
            <input class="btnimg collectBtn" src="/img/btn/b01-search3.gif" type="image" alt="キーワード検索" />
        </div>
        {{ Form::close() }}
        {{-- </form> --}}
        ...
        @if (count($keywords) > 0)
            <ul>
                ...
                @foreach ($keywords as $keyword)
                    <li>
                        {{-- <a href="@if (!is_null($keyword['value'])) {$v->env.path_base}{$keyword.value} @else {$v->env.path_base}/keywords/?keywords={$keyword.word|urlencode} @endif ">{{ $keyword['word'] }}</a> --}}
                        {{--
                            MEMO:
                            $keyword['value'] には、null でなければ、 '/rentacar/', '/feature/ninki/' などのように
                            url が入っているらしい。
                         --}}
                        <a href="{{ $keyword['value'] ?? route('rsv.keywords.search', ['keywords' => $keyword['word']]) }}">{{ $keyword['word'] }}</a>
                    </li>
                @endforeach
            </ul>
        @endif
