...
class BrpartnerController extends _commonController
{
    ...
    public function partnerConf()
    {
        ...
        if ($return_from_update != "true") {
            ...
            $a_row['open_ymd']
                = "$date->year" . "/" . sprintf('%02d', $date->month) . "/" . sprintf('%02d', $date->day);
            //初期表示時に検索結果をassign
    ...
    public function partnerControlEdt()
    {
        ...
        // 接続形態一覧の取得
        $a_connect_cls_list = $models_partner->getConnectClsList();

        // 接続形態（詳細）一覧の取得
        $a_connect_type_list = $models_partner->getConnectTypeList();
    ...
    public function partnerControlUpd()
    {
        ...
        if ($requestPartnerControl['is_send_report'] == 1) {
            ...
            //----------------------------------------------------------
            // 空行は削除
            foreach (($requestPartnerControl['result_email_list'] ?? []) as $key => $value) {
        ...
        // 画面入力を変換 （TODO 入力でも使うはず）
        $errorList = $this
            ->validatePartnerControlFromScreen($partnerControlData, $requestPartnerControl, $partnerControlModel);
