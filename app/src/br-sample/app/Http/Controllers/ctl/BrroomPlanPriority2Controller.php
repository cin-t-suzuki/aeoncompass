...
class BrroomPlanPriority2Controller extends _commonController
{
	...
	use Traits;
	private $_priority_cnt = 10; // 重点表示順位の登録表示件数
	private $_a_wday = array(2 => "月曜",3 => "火曜",4 => "水曜",5 => "木曜",6 => "金曜",7 => "土曜", 1 => "日曜");  // tplでは月曜日始まりなので2から始まる ※wday 1:日曜 2:月曜 3:火曜 4:水曜 5:木曜 6:金曜 7:土曜
	...
	{
		//初期化
		$a_select_pref = array(); //追記
		$a_priority_list = array(); //追記
		...
		if (!$this->is_empty($a_priority)) {
			...
			// 重点表示プラン一覧の取得
			$a_room_plan_priorities = $planModel->get_room_plan_priorities($a_conditions);
			...
			if (!$this->is_empty($a_priority_list)) {
				...
				foreach ($a_priority_list as $key => $value) {
					foreach ($value as $key2 => $value2) {
						...
						$planData = $roomPlanModel->selectByTripleKey($value2['hotel_cd'],$value2['room_cd'],$value2['plan_cd']);

						$a_priority_list[$key][$key2]['hotel_nm'] = mb_substr($hotelData['hotel_nm']??null, 0, 12); //null追記
						$a_priority_list[$key][$key2]['room_nm']  = mb_substr($roomData['room_nm']??null, 0, 12); //null追記
						$a_priority_list[$key][$key2]['plan_nm']  = mb_substr($planData['plan_nm']??null, 0, 12); //null追記
					}
				}
			}
		}
	...
	public function registration()
	{
		...
		// _a_wday 1:日曜 2:月曜 3:火曜 4:水曜 5:木曜 6:金曜 7:土曜
		// tplでは月曜日始まりなので2から始まる
		foreach ($this->_a_wday as $wday => $week_nm){

			// 施設コード、	部屋コード、プランコードの何れかが存在する場合はエラーチェック。
			// チェック後に登録or更新を行う
			//一旦下記3か所をis_emptyに変えてみる＆null追記（zap~は今後使わないと元ソースで追記アリ）
			if (!$this->is_empty($a_priority['hotel_cd'.$wday."_".$a_priority['priority']]??null) 
			...
			|| !$this->is_empty($a_priority['plan_cd'.$wday."_".$a_priority['priority']]??null)){
				...
				// 重点プラン用存在チェック
				$res = $this->_existence_check($wday, $week_nm, $a_priority);
				...
				// 重点表示部屋プランの取得＆設定
				$a_room_plan_priority = $roomPlanPriorityModel->selectBy4Key($a_priority['pref_id'],$a_priority['span'],$wday,$a_priority['priority']);
				...
				if ($this->is_empty($a_room_plan_priority)){
					...
					$a_room_plan_priority['hotel_cd']  = $a_priority['hotel_cd'.$wday."_".$a_priority['priority']];
					...
					$a_room_plan_priority['display_status']  = $a_priority['display_status'.$wday."_".$a_priority['priority']];
			...
			// 施設コード、	部屋コード、プランコードが存在しない場合はレコードを削除
				...
				// 削除
				$roomPlanPriorityModel->deleteBy4Key($a_priority['pref_id'],$a_priority['span'],$wday,$a_priority['priority']);
	...
	//	
	private function _existence_check($wday, $week_nm, $aa_priority){
	...
	public function sort()
	{
		...
		$this->_sort_list($pref_id,$span,$priority,"0",$modify_cd,$modify_ts);
		$this->_sort_list($pref_id,$span,$other_priority,$priority,$modify_cd,$modify_ts);
		$this->_sort_list($pref_id,$span,"0",$other_priority,$modify_cd,$modify_ts);
	...
	// 並べ替え登録
	//=====================================================================
	protected function _sort_list($pref_id,$span,$priority,$other_priority,$modify_cd,$modify_ts)
	{
		try {
			$s_sql = //modify関係書き換え
<<< SQL
