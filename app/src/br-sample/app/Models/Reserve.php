										...
										(
											...
											from	reserve_extension,
												(
													...
													where	reserve_plan_info.reserve_cd = q4.reserve_cd -- (+)は削除でいいか
												) q5
											where	reserve_extension.reserve_cd = q5.reserve_cd -- (+)は削除でいいか
										) q6
									where	reserve_contact.reserve_cd = q6.reserve_cd -- (+)は削除でいいか
										{$s_email}
								) q7
							where	reserve_insurance_weather.reserve_cd = q7.reserve_cd -- (+)は削除でいいか
								and	reserve_insurance_weather.date_ymd   = q7.date_ymd -- (+)は削除でいいか
								{$s_insurance_weather}
						) q8
					where	null is null
    ...
    private function hasVoice($aa_conditions)
    {
        ...
        try {
            ...
            <<< SQL
					...
					where	voice_reply.hotel_cd = q1.hotel_cd -- (+)は削除でいいか？
						and	voice_reply.voice_cd = q1.voice_cd -- (+)は削除でいいか？
SQL;
