...
class Partner extends CommonDBModel
{
    ...
    public function __construct() //function→public functionでいいか？（phpcs赤エラー）
    {
        ...
        $colPartnerNm = (new ValidationColumn())->setColumnName($this->COL_PARTNER_NM, "提携先名称")
            ->require()->length(0, 65)->notHalfKana();
        $colSystemNm = (new ValidationColumn())->setColumnName($this->COL_SYSTEM_NM, "システム名称")
            ->require()->length(0, 65)->notHalfKana();
        $colPartnerNs = (new ValidationColumn())->setColumnName($this->COL_PARTNER_NS, "提携先略称")
            ->require()->length(0, 20)->notHalfKana();
        $colUrl = (new ValidationColumn())->setColumnName($this->COL_URL, "ウェブサイトアドレス")
            ->require()->length(0, 128)->notHalfKana()->url();
        $colPostalCd = (new ValidationColumn())->setColumnName($this->COL_POSTAL_CD, "郵便番号")
            ->require()->length(0, 8)->notHalfKana()->postal();
        $colAddress = (new ValidationColumn())->setColumnName($this->COL_ADDRESS, "住所")
            ->require()->length(0, 100)->notHalfKana();
        $colTel = (new ValidationColumn())->setColumnName($this->COL_TEL, "電話番号")
            ->require()->length(0, 15)->notHalfKana()->phoneNumber();
        $colFax = (new ValidationColumn())->setColumnName($this->COL_FAX, "ファックス番号")
            ->require()->length(0, 15)->notHalfKana()->phoneNumber();
        $colPersonPost = (new ValidationColumn())->setColumnName($this->COL_PERSON_POST, "担当者役職")
            ->require()->length(0, 32)->notHalfKana();
        $colPersonNm = (new ValidationColumn())->setColumnName($this->COL_PERSON_NM, "担当者名称")
            ->require()->length(0, 32)->notHalfKana();
        $colPersonKn = (new ValidationColumn())->setColumnName($this->COL_PERSON_KN, "担当者かな名称")
            ->require()->length(0, 64)->notHalfKana()->HiraganaOnly();
        $colPersonEmail = (new ValidationColumn())->setColumnName($this->COL_PERSON_EMAIL, "担当者電子メールアドレス")
            ->require()->length(0, 128)->notHalfKana()->emails();
        $colOpenYmd = (new ValidationColumn())->setColumnName($this->COL_OPEN_YMD, "公開日")
            ->require()->correctDate();
    ...
    public function getPartners($aa_conditions = [])
    {
...
SQL;
    ...
    // 接続形態一覧の取得
    //======================================================================
    public function getConnectClsList()
    {
        // 初期化
        $a_return = [];
...
SQL;
        ...
        // 整形
        foreach (($a_rows ?? []) as $a_row) {   //nvl($a_rows, array())空の書き換え合っている？
            // 接続形態がnullのものは弾く
    ...
    // 接続形態（詳細）一覧の取得
    //======================================================================
    public function getConnectTypeList()
    {
        // 初期化
        $a_return = [];
