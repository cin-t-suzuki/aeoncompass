...
class PartnerKeyword extends CommonDBModel
{
    ...
    public function __construct() //function→public functionでいいか？（phpcs赤エラー）
    {
        ...
        $colLayoutType = (new ValidationColumn())->setColumnName($this->COL_LAYOUT_TYPE, "表示場所")
            ->require()->length(0, 9)->intOnly();
        $colBranchNo = (new ValidationColumn())->setColumnName($this->COL_BRANCH_NO, "枝番")
            ->require()->length(0, 2)->intOnly();
        $colWord = (new ValidationColumn())->setColumnName($this->COL_WORD, "表示文字列")
            ->length(0, 32)->notHalfKana();
        $colValue = (new ValidationColumn())->setColumnName($this->COL_VALUE, "値")
            ->length(0, 32)->notHalfKana();
        $colOrderNo = (new ValidationColumn())->setColumnName($this->COL_ORDER_NO, "表示順序")
            ->length(0, 10)->intOnly();
        $colDisplayStatus = (new ValidationColumn())->setColumnName($this->COL_DISPLAY_STATUS, "表示ステータス")
            ->require(); // TODO パターンチェック必要？カラムの説明
    ...
    public function list($request_params)
    {
        ...
        $list = $this->getList($request_params);
        return $list;
    }
    ...
    // 一覧の取得
    //======================================================================
    private function getList($request_params)
    {
        // 初期化
        $s_sql        = '';
        $a_conditions = [];
...
SQL;
    ...
    //
    //======================================================================
    public function displayStatusSelecter()
    {
        // 検索用パラメータ設定（パートナー管理に戻った時用）
        $display_status_selecter = $this->getDisplayStatusSelecter();
    ...
    // 表示設定の選択項目を作成
    //======================================================================
    private function getDisplayStatusSelecter()
    {
        // 初期化
        $result    = [];
        $names = [];
    ...
    //
    //======================================================================
    public function countRecord($request_params)
    {
        $display_status_selecter = $this->getCountRecord($request_params);
        return $display_status_selecter;
    }
    ...
    //======================================================================
    private function getCountRecord($request_params)
    {
        try {
            // 初期化
            $s_sql        = '';
...
SQL;
            ...
            return (int)$a_result[0]->countRecord;
        } catch (Exception $e) {
            throw $e;
        }
    }
    ...
    {
        $data = $this->where([
            $this->COL_PARTNER_CD => $partner_keyword_example_work['partner_cd'],
            $this->COL_LAYOUT_TYPE => $partner_keyword_example_work['layout_type'],
            $this->COL_BRANCH_NO => $partner_keyword_example_work['branch_no']])
            ->get();
    ...
    public function updateByTripleKey($con, $partnerKeywordData)
    {
        $result = $con->table($this->table)->where([
                $this->COL_PARTNER_CD => $partnerKeywordData['partner_cd'],
                $this->COL_LAYOUT_TYPE => $partnerKeywordData['layout_type'],
                $this->COL_BRANCH_NO => $partnerKeywordData['branch_no']])
                ->update($partnerKeywordData);
        return  $result;
    }
}
