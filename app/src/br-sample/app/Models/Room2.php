...
class Room2 extends CommonDBModel
{
    ...
    // 独自のバリデーション
    //
    //     public function capacity_min_validate()
    //     {
    //         $o_validator = Validations::getInstance($this->box);
    //         $o_column = $this->get_column_object('capacity_min');

    //         if (!(ceil($this->_a_attributes['capacity_min']) == $this->_a_attributes['capacity_min'] && abs($this->_a_attributes['capacity_min']) == $this->_a_attributes['capacity_min'])) {
    //             return '最小定員はマイナス、小数点を含まない１以上の数値を入力してください。';
    //         }

    //         if ($this->_a_attributes['capacity_min'] <= 0) {
    //             return '最小定員はマイナス、小数点を含まない１以上の数値を入力してください。';
    //         }
    //     }

    //     public function capacity_max_validate()
    //     {
    //         $o_validator = Validations::getInstance($this->box);
    //         $o_column = $this->get_column_object('capacity_max');


    //         if (!(ceil($this->_a_attributes['capacity_max']) == $this->_a_attributes['capacity_max'] && abs($this->_a_attributes['capacity_max']) == $this->_a_attributes['capacity_max'])) {
    //             return '最大定員はマイナス、小数点を含まない１以上の数値を入力してください。';
    //         }

    //         if ($this->_a_attributes['capacity_max'] <= 0) {
    //             return '最大定員はマイナス、小数点を含まない１以上の数値を入力してください。';
    //         }
    //     }

    //     // 部屋ラベル
    //     public function label_cd_validate()
    //     {
    //         $o_validator = Validations::getInstance($this->box);
    //         $o_column = $this->get_column_object('label_cd');

    //         if (empty($this->_a_attributes['label_cd'])) {
    //             return null;
    //         }
    ...
    // 					and	upper(label_cd) = upper(:label_cd)
    // 					and	room_id != :room_id
    // 					and	display_status = 1
    // SQL;
    //         $_oracle = _Oracle::getInstance();
    //         $a_room2 = $_oracle->find_by_sql($s_sql, array('hotel_cd' => $this->_a_attributes['hotel_cd'], 'label_cd' => $this->_a_attributes['label_cd'], 'room_id' => $this->_a_attributes['room_id']));

    //         if (!empty($a_room2[0]['hotel_cd'])) {
    //             return $o_column->get_logical_nm() . '（' . $this->_a_attributes['label_cd'] . '）はすでに登録されています。';
    //         }
    //     }
}
