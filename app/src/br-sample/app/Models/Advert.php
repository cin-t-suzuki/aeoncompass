...
class Advert extends CommonDBModel
{
    ...
    {
        // // カラム情報の設定
        $colRecordId = (new ValidationColumn())
            ->setColumnName($this->COL_RECORD_ID, "広告掲載ID")->require()->length(0, 8)->intOnly();
        $colHotelCd = (new ValidationColumn())
            ->setColumnName($this->COL_HOTEL_CD, "施設コード")->require()->length(0, 10)->notHalfKana(); //TODO 独自チェック
        $colAdvertSYmd = (new ValidationColumn())
            ->setColumnName($this->COL_ADVERT_S_YMD, "掲載開始年月日")->require()->hyphenOrSlashDate();
        $colAdvertEYmd = (new ValidationColumn())
            ->setColumnName($this->COL_ADVERT_E_YMD, "掲載最終年月日")->require()->hyphenOrSlashDate();
        $colAdvertOrder = (new ValidationColumn())
            ->setColumnName($this->COL_ADVERT_ORDER, "掲載順序")->require()->length(0, 8)->intOnly();
        $colAdvertCharge = (new ValidationColumn())
            ->setColumnName($this->COL_ADVERT_CHARGE, "掲載金額")->require()->length(0, 7)->intOnly();
        $colAdvertStatus = (new ValidationColumn())
            ->setColumnName($this->COL_ADVERT_STATUS, "掲載状態")->length(0, 1)->intOnly(); //TODO パターンチェック、カラムの説明
    ...
    //
    //  aa_conditions
    public function getHotelAdvert2009000400s($aa_conditions = [])
    {
