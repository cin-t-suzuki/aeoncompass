...
class PartnerControl extends CommonDBModel
{
    ...
    public function __construct() //function→public functionでいいか？（phpcs赤エラー）
    {
        ...
        $colPwAdmin = (new ValidationColumn())->setColumnName($this->COL_PW_ADMIN, "管理パスワード")
            ->require()->length(0, 64)->notHalfKana();
        $colPwOperator = (new ValidationColumn())->setColumnName($this->COL_PW_OPERATOR, "運用パスワード")
            ->require()->length(0, 64)->notHalfKana();
        $colPwUser = (new ValidationColumn())->setColumnName($this->COL_PW_USER, "接続パスワード")
            ->length(0, 12)->notHalfKana(); //パスワード系、ひらがなとかはＯＫでいいの？
        $colCharset = (new ValidationColumn())->setColumnName($this->COL_CHARSET, "キャラクタセット")
            ->length(0, 16)->notHalfKana(); // TODO パターンチェック必要？カラムの説明
        $colVoiceStatus = (new ValidationColumn())->setColumnName($this->COL_VOICE_STATUS, "掲示板表示設定"); // TODO パターンチェック必要？
        $colPageTimelimit = (new ValidationColumn())->setColumnName($this->COL_PAGE_TIMELIMIT, "ページ有効時間")
            ->length(0, 4)->intOnly();
        $colExtensionState = (new ValidationColumn())->setColumnName($this->COL_EXTENSION_STATE, "付随情報表示状態"); // TODO パターンチェック必要？
        $colSalesType = (new ValidationColumn())->setColumnName($this->COL_SALES_TYPE, "販売可能タイプ")
            ->require()->intOnly();
        $colAuthType = (new ValidationColumn())->setColumnName($this->COL_AUTH_TYPE, "予約照会認証タイプ")
            ->require(); // TODO パターンチェック必要？カラムの説明
        $colRate = (new ValidationColumn())->setColumnName($this->COL_RATE, "料率")->length(0, 3)->intOnly();
        $colResultEmail = (new ValidationColumn())->setColumnName($this->COL_RESULT_EMAIL, "実績レポート配信メールアドレス")
            ->length(0, 128)->notHalfKana()->emails();
        $colFromNm = (new ValidationColumn())->setColumnName($this->COL_EMAIL_FROM_NM, "お客様向けメール送信者名称")
            ->length(0, 15)->notHalfKana();
        $colRpcStatus = (new ValidationColumn())->setColumnName($this->COL_RESULT_RPC_STATUS, "予約時実績報告レポート配信")
            ->require()->intOnly();  // TODO パターンチェック必要？カラムの説明
        $colRpcUrl = (new ValidationColumn())->setColumnName($this->COL_RESULT_RPC_URL, "予約時実績報告レポート配信先URL")
            ->length(0, 256)->url();
