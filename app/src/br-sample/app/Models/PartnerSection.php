...
class PartnerSection extends CommonDBModel
{
	...
	function __construct(){
		...
		$colSectionId= (new ValidationColumn())->setColumnName($this->COL_SECTION_ID, "所属団体ID")->require()->length(0,2)->intOnly();
		$colSectionNm= (new ValidationColumn())->setColumnName($this->COL_SECTION_NM, "所属団体名称")->require()->length(0,32)->notHalfKana();
	...
	// 所属団体一覧の取得
	//======================================================================
	public function get_section_list($request_params) //trycatch一旦削除したがOK？引数追加
	{
			// 初期化
			$s_sql        = '';
			$a_conditions = array();
...
SQL;
	...
	public function updateByWKey($con, $partnerSectionData){
		$result = $con->table($this->table)->where(array($this->COL_PARTNER_CD=>$partnerSectionData['partner_cd'],$this->COL_SECTION_ID=>$partnerSectionData['section_id']))->update($partnerSectionData);
		return  $result;
	}
	...
	public function deleteByKey($con, $a_partner_section_work){
		$result = $con->table($this->table)->where(array($this->COL_PARTNER_CD=>$a_partner_section_work['partner_cd'],$this->COL_SECTION_ID=>$a_partner_section_work['section_id']))->delete();
		return $result;
	}
	...
	public function selectByWKey($a_partner_section_work){
		$data = $this->where(array($this->COL_PARTNER_CD=>$a_partner_section_work['partner_cd'],$this->COL_SECTION_ID=>$a_partner_section_work['section_id']))->get();
	...
	// 所属団体IDの取得(登録用)
	//======================================================================
	public function get_section_id_next($_a_request_params)
	{
		try {
			// 初期化
			$s_sql        = '';
...
SQL;
	...
	// 表示順の取得(登録用)
	//======================================================================
	public function get_order_no_next($_a_request_params)
	{
		try {
			// 初期化
			$s_sql        = '';
