...
abstract class CommonDBModel extends Model
{
    ...
    public function validation($values)
    {
        ...
        foreach ($values as $key => $val) {
            ...
            // ひらがなのみかチェック
            if ($this->colmunArray[$key]->isHiraganaOnly()) {
                if (strlen($val) != 0) {
                    if (!preg_match('/^[ぁ-ゞー　\s]+$/u', $val)) {
                        $rtnErrors[] = $this->colmunArray[$key]->getColumnName() . "にひらがな以外が含まれています";
                    }
                }
            }
            ...
            if ($this->colmunArray[$key]->isUrl()) {
                if (strlen(($val) != 0)) {
                    if (!$this->is_url($val)) {
                        $rtnErrors[] = $this->colmunArray[$key]->getColumnName() . "アドレスを半角で正しく入力してください。";
                    }
                }
            }
            ...
            // 半角英数のみであるかチェック
            if ($this->colmunArray[$key]->isNumberAndAlphabet()) {
                if (strlen($val) != 0) {
                    if (!preg_match('/^[0-9A-Za-z]+$/', $val)) {
                        $rtnErrors[] = $this->colmunArray[$key]->getColumnName() . "は半角英数で入力してください";
                    }
                }
            }
