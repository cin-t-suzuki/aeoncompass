...
class RoomPlanPriority extends CommonDBModel
{
	...
	function __construct(){
		...
		parent::setColumnDataArray([$colPrefId,$colSpan,$colWday,$colPriority,$colHotelCd,$colRoomCd,$colPlanCd,$colDisplayStatus]);
	}
	...
	public function selectBy4Key($prefId,$span,$wday,$priority){
		$data = $this->where(array($this->COL_PREF_ID=>$prefId,$this->COL_SPAN=>$span,$this->COL_WDAY=>$wday,$this->COL_PRIORITY=>$priority))->get();
	...
	public function updateBy4Key($con, $roomPlanPriorityData){
		$result = $con->table($this->table)->where(array($this->COL_PREF_ID=>$roomPlanPriorityData['pref_id'],$this->COL_SPAN=>$roomPlanPriorityData['span'],$this->COL_WDAY=>$roomPlanPriorityData['wday'],$this->COL_PRIORITY=>$roomPlanPriorityData['priority']))->update($roomPlanPriorityData);
		return  $result;
	}
	...
	public function deleteBy4Key($prefId,$span,$wday,$priority){
		$result = $this->where(array($this->COL_PREF_ID=>$prefId,$this->COL_SPAN=>$span,$this->COL_WDAY=>$wday,$this->COL_PRIORITY=>$priority))->delete();
		return $result;
	}
