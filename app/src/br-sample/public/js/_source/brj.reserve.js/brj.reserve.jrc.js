		...
		initialize: function(){
						...
						if(jf.length > 0){
							...
							if (0 <= d.getHours() && d.getHours() < 12) {
								var ds = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 3);
							} else {
								var ds = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 4);
							}
							var df = new Date(d.getFullYear(), d.getMonth() + 2, d.getDate()		);
							var dn = new Date(d.getFullYear(), d.getMonth() + 2, 1							);
							while(df.getFullYear() != dn.getFullYear() || df.getMonth() != dn.getMonth()){
							...
							jym.each(function(){
								checkin.setYearMonthOptions(this, startYMD, finalYMD, defaultYMD);
							});
							jym.change(function(){
								eventChangeYM(this);
							});
							...
							if(jdict.length + jpref.length > 0){
								...
								if(m){ // 地図の大エリア（地方）
									...
									if(m){ // 地図の都道府県エリア
										spref = (m[1] * 1) + ''; sdict = area.getLAreaParentP(spref).i;
									}else{
										var m = spath.match(/\/area\/m([0-9]{1,3})\/$/);
										if(m){ // 地図の中エリア
											spref = area.getPAreaParentM(m[1]).i; sdict = area.getLAreaParentP(spref).i;
										}
									}
								}
	...
	var eventSubmitForm = function(of){
					...
					$(':input', $(of)).each(function(){
						...
						if(sv.length > 0){
							...
							else if(sn == 'dept'		){ p.Departure = sv; } // 出発地（Departure）
							else if(sn == 'dict'		){ p.Direction = area.getLArea(sv).ia; } // 方面（Direction）
							else if(sn == 'pref'		){ p.Pref 		 = ('0' + sv).slice(-2); } // 都道府県(Pref）
							else if(sn == 'area'		){ p.Area 		 = sv; } // エリア（Area）
							else if(sn == 'guest' 	){ p.Pax			 = sv; } // 1室人数（Pax）
							else if(sn == 'hotel_cd'){ p.HotelCD	 = sv; } // 施設コード（HotelCD）
	...
	var setAreaOptions = function(os, sdef, spref){
					...
					if(spref != 47){
						for(var i in m){
							...
							if(m[i].ia == sdef) os.options.selectedIndex = os.options.length - 1;
						}
					}
	...
	var setDictOptions = function(os, sdef){
					...
					for(var i in l){
						if(i != 12){
							...
							if(l[i].i == sdef) os.options.selectedIndex = os.options.length - 1;
						}
					}
	...
	var setPrefOptions = function(os, sdef, sdict){
					...
					for(var i in p){
						if(i != 47){
							...
							if(p[i].i == sdef) os.options.selectedIndex = os.options.length - 1;
						}
					}
